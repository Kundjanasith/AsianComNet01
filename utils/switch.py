...
class Switch(): # ACTION DESTINATION STATE --> current packet size, remainin_time
    ...
    def randomForward(self, packet, t, mode): #MOCK FORWARD TO DST

        if mode == 'SHORTEST':
            print(packet.src, packet.dst, packet.paths)
            sys.exit()
        ...
        if mode == 'DRL':
            ...
            if np.random.rand() < self.epsilon:
                forwardTo = random.choice(list(self.outConnections.keys()))
            else:
                ...
                # print(len(action_probs[0]))
                action = np.argmax(action_probs[0])
                forwardTo = list(self.outConnections.keys())[action]

            if self.outConnections[forwardTo].packet != None:
                for i in range(len(self.outConnections.keys())):
                    tmp = list(self.outConnections.keys())
                    if self.outConnections[forwardTo].packet == None:
                        break 
                    else:
                        tmp.remove(forwardTo)
                        forwardTo = random.choice(tmp)

            # print(self.name, forwardTo, self.outConnections.keys())
            ...
            if forwardTo in self.outConnections.keys():
                if self.outConnections[forwardTo].packet == None:
